...
/*-----------------Delta for check SQN ----------------------------*/
/*----- Delta shold greater than the number of AV  -------*/
unsigned long delta;

/*--------------------------- prototypes --------------------------*/

// Check the machine is big endian or little endian
...
void genSQN(u8 oldSQN[6], u8 newSQN[6])
{
    ...
    //ISCUtil::binToHexStr((char *)buf, 8, (char *)showBuf, 17, false, false);
    //printf("new buffer is :%s:\n",  showBuf);
    memcpy(&tmp, buf, 8);
    tmp++;
    //ISCUtil::binToHexStr((char *)&tmp, 8, (char *)showBuf, 17, false, false);
    //printf("new buffer is :%s:\n",  showBuf);
    u8* p = (u8*)&tmp;
    memcpy(newSQN, p + 2, 6);
    //ISCUtil::binToHexStr((char *)newSQN, 6, (char *)showBuf, 17, false, false);
    //printf("new buffer is :%s:\n",  showBuf);
}

bool chkSQN(u8 oldSQN[6], u8 newSQN[6])
{
    unsigned long long oldL;
    ...
    memcpy(&oldL, oldBuf, 8);
    memcpy(&newL, newBuf, 8);
    // This line "usigned long long max = 0xFFFFFFFFFFFF;" won't compile on MV Linux.
    ...
    else if (((newL <= oldL + delta) && (newL >= oldL)) || (newL - oldL + max <= delta))
    {
        return true;
    }
    return false;
...
void genAuts(u8 k[16], u8 rand[16], u8 ue_sqn[6], u8 amf[2], u8 auts[14])
{
    // AUTS = (SQN^AK)||MAC_S
    u8 mac_s[8], ak[6];
    f1star(k, rand, ue_sqn, amf, mac_s);
    f5star(k, rand, ak);
    for (int i = 0; i < 6; i++)
    {
        auts[i] = ue_sqn[i] ^ ak[i];
    }
    for (int i = 6; i < 14; i++)
    {
        auts[i] = mac_s[i - 6];
 ...
 *-------------------------------------------------------------------
 *
 *  Takes K, and RAND||AUTS from the USIM. Outputs the SQNms, RAND
 *  sent from the USIM and MAC_S sent from the USIM.
 *
 *-----------------------------------------------------------------*/

void getResyncInputs(u8 k[16], u8 rand_auts[30], u8 sqn_ms[6], u8 rand_usim[16], u8 mac_s_usim[8])
{
    // Isolate RAND, and AUTS
    memcpy(rand_usim, rand_auts, 16);

    // Isolate AUTS.
    // AUTS = conc(SQNms)||MAC-S
    u8 auts[14];
    memcpy(auts, rand_auts + 16, 14);

    // Isolates Conc(SQNms) and MAC-S
    u8 conc_sqn_ms[6];
    memcpy(conc_sqn_ms, auts, 6);
    memcpy(mac_s_usim, auts + 6, 8);

    // Computes sqn_ms
    // SQNms = Conc(SQNms)^f5star(RAND)

    u8 ak[6];
    f5star(k, rand_usim, ak);
    for (int i = 0; i < 6; i++)
    {
        sqn_ms[i] = conc_sqn_ms[i] ^ ak[i];
    }
}
